...
function makeDbMock(state: DbState) {
  // select(...).from(...).where(...).limit(...) -> Promise<rows>
  const limit = vi.fn().mockImplementation(async () => []);
  const where = vi.fn().mockImplementation((cond: any) => {
    const lookupName = cond?.val;
    return {
      limit: vi.fn().mockImplementation(async () =>
        state.existingNames.has(lookupName) ? [{ id: "existing-id" }] : []
      ),
    };
  });
  ...
  const values = vi.fn().mockImplementation(async (row: Record<string, unknown>) => {
    state.inserted.push(row);
  });
  const insert = vi.fn().mockReturnValue({ values });

  return { select, insert, limit, where, from, values } as any;
}
...
describe("seedSkills", () => {
  ...
  it("inserts all skills on first run", async () => {
    const result = await seedSkills(db, {
      ...
      readFileText: async (p: string) =>
        p.endsWith("candlestick.md") ? VALID_MD : VALID_MD_2,
    });
  ...
  it("counts a single bad file as failed but continues with the rest", async () => {
    ...
    const result = await seedSkills(db, {
      ...
      readFileText: async (p: string) => {
        if (p.endsWith("bad.md")) throw new Error("disk on fire");
        if (p.endsWith("good.md")) return VALID_MD;
        return VALID_MD_2;
      },
      logger,
    });
