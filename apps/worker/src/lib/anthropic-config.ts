...
export function getAnthropicConfig(scenario: AnthropicScenario): AnthropicConfig {
  ...
  return {
    apiKey:
      process.env[`ANTHROPIC_API_KEY_${scenario}`] || process.env.ANTHROPIC_API_KEY || undefined,
    baseURL:
      process.env[`ANTHROPIC_BASE_URL_${scenario}`] || process.env.ANTHROPIC_BASE_URL || undefined,
    model:
