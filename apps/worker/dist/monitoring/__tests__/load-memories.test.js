...
import { mergeAndCapMemories } from "../load-memories.js";
...
describe("mergeAndCapMemories", () => {
    ...
    it("caps to 8 entries", () => {
        const many = Array.from({ length: 15 }, (_, i) => mk({ id: `m${i}`, updatedAt: new Date(NOW - i * 1000) }));
    ...
    it("respects total 4000 char budget", () => {
        const big = Array.from({ length: 8 }, (_, i) => mk({ id: `m${i}`, content: "a".repeat(800) }));
