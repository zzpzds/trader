...
function createMockChild(stdout, stderr, exitCode) {
...
describe("fetchPrices", () => {
    ...
    it("returns parsed price data on success", async () => {
        ...
        expect(child.stdin.write).toHaveBeenCalledWith(JSON.stringify({ symbols: ["QQQ"], period: "60d" }));
    });
    ...
    it("uses custom period when provided", async () => {
        ...
        expect(child.stdin.write).toHaveBeenCalledWith(JSON.stringify({ symbols: ["SPY"], period: "30d" }));
    });
});
