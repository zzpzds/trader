...
export async function upsertSnapshots(db, result) {
...
export async function ensurePriceSnapshots(db, symbol, fromDate) {
    const existing = await db
        .select({ minDate: min(priceSnapshots.date) })
        .from(priceSnapshots)
        .where(eq(priceSnapshots.symbol, symbol));
    const existingMin = existing[0]?.minDate ?? null;
    if (existingMin && existingMin <= fromDate)
        return;
    const daysBack = Math.ceil((Date.now() - new Date(fromDate).getTime()) / 86_400_000) + 1;
